...
if platform.system() == 'Darwin':
    ...
    txt2smf_app = "/usr/local/bin/txt2smf"
    weka_app = "java -cp /usr/share/java/weka.jar"
else:
    smf2txt_app="/usr/local/bin/smf2txt"
    txt2smf_app = "/usr/local/bin/txt2smf"
    weka_app = "java -cp /Users/octavio/bin/weka.jar"
...
class TTrack:
    ...
    def _get_duration(self):
        return float(self.skyline[len(self.skyline)-1]["onset"]-self.skyline[0]["onset"] \
                     + self.skyline[len(self.skyline)-1]["duration"])
...
class Midi:
    ...
    # class_ok: lista con los numeros de pista que pertenecen a la clase 
    # (pe: class_ok=[3,4]. En el caso del bajo ,las pistas 3 y 4 don de bajo)
...
class TArff:
    def __init__(self,filename,class_ok):        
        ...
        self.midifile = Midi()
        self.midifile.load_midi(filename)
