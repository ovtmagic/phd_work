...
for i in loo_list:
    ...
    print "File:%s,%s,%s,%s,%s,%s,%s" % (midi_filename, track_type_bass, bass_track, TP, FP, FN, TN)
...
print "Total:%s;%s;%.2f%%;%s;%s;%s;%s;%s;%s;%s;%s;%.2f;%.2f;%.2f" % (csv_filename.split("/")[-1], track_type_bass, accuracy, total_errors[0], total_errors[1], total_errors[2], total_errors[3], confusion_matrix['TP'], confusion_matrix['FP'], confusion_matrix['FN'],  confusion_matrix['TN'], precision, recall, f_m)
