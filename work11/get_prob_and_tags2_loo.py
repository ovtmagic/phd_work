...
for i in loo_list:
    ...
    for t in m.tracks.keys():
        ...
        if m.tracks[t].is_valid():
            oc = "%.2f" % round(m.tracks[t]._get_occupation_rate(),2)
            prob_track_m = "%.2f" % round(prob_track_m, 2)
            prob_track_b = "%.2f" % round(prob_track_b, 2)
            prob_track_a = "%.2f" % round(prob_track_a, 2)
        else:
            prob_track_m=""
            prob_track_b=""
            prob_track_a=""
        ...
        if format_csv:
            ...
            print "   %2s   %4s   %4s   %4s   %4s  %4s %-9s  %s" % (t, selected, prob_track_m, prob_track_b, prob_track_a, oc, tag, m.tracks[t].name)
