...
def get_probabilites(train_filename, test_filename, debug=False):
    ...
    if( re.search(".model$",train_filename) ):
        ...
        #cmd="java -cp /Users/octavio/bin/weka.jar weka.classifiers.trees.RandomForest -l %s -T %s -p 0 -distribution" % (train_filename, test_filename)
    else:
...
#===============================================================================
class Tsmf2txt:
    def __init__(self,filename,load_skyline=True,format='midi'):
    ...
    def read_file(self,is_skyline=False):
        ...
        if(is_skyline):
            if self.format == 'midi':
                ...
                f=open(self.filename+".sky")
            #print ">>>>>>>>",self.smf2txt+" -p 1 "+self.filename
            #f=subprocess.Popen(self.smf2txt+" -p 1 "+self.filename)
            #f=subprocess.Popen(['/usr/local/bin/smf2txt','-f%p %o %d %v %c','-p1',self.filename], stdout=subprocess.PIPE)
            ...
            #print ">>>>>>>>",self.smf2txt+" -p 1 "+self.filename
            #f=subprocess.Popen(self.smf2txt+" "+self.filename)
            #f=subprocess.Popen(['/usr/local/bin/smf2txt','-f%p %o %d %v %c',self.filename], stdout=subprocess.PIPE)
        content=f.read()
        f.close()
        #content=f.communicate()[0]
...
class Ttxt2smf:
    ...
    def __init__(self, arg1, resolution=1):
        self.tracks = {}
        self.txt2sms = "/usr/local/bin/txt2smf"
