...
class Tsmf2txt:
    def __init__(self,filename,load_skyline=True):
        ...
        # Comrprueba si tiene que obtener el skyline
        self.skyline_readed=False
        if(load_skyline):
            self.readSkyline()
        # indica si se han obtenido los descriptores
        self.decriptos_obtained=False
        # nombres de la clase para el fichero arff
        self.class_name=['yes','no']
        self.arff_head=""
        self.arff=""
        #lista de descriptores para contruir el arff
        self.descriptors=[
            'avg_poliphony',
            'avg_poliphony_norm',
            'duration_norm',
    ...
    # devuelve el numero de pista a partir del numero de orden
    def get_numbyord(self,ord):
        if(ord==-1):
            return -1
        for i in self.tracks.keys():
            if self.tracks[i].ord==ord:
                return self.tracks[i].number
        return -1
    def gno(self,ord):
        return self.get_numbyord(ord)
