...
if(len(sys.argv) == 6):
    ...
    print """ ./track_selection.py <bass|melody> <threshold> <train_file.arff> <csv_file> <path_to_files>
        <bass|melody>
        <threshold>
        <train_file.arff>: fichero de weka de entrenamiento
        <csv_file>: fichero con las etiquetas
...
print "Total %s;%s;%s;%.2f%%;%s;%s;%s;%s;%s;%s;%s;%s;%.2f;%.2f;%.2f" % (train_filename.split("/")[-1],csv_filename.split("/")[-1], 
