...
#    <a_train_file.arff>: fichero de weka entrenado con acompanamiento
...
if(len(sys.argv) == 6):
    ...
    print """ ./get_prob_and_tags.py <m_train_file.arff> <b_train_file.arff> <a_train_file.arff> <csv_file> <path_to_files>
        <m_train_file.arff>: fichero de weka entrenado con melodia
        <b_train_file.arff>: fichero de weka entrenado con bajo
        <a_train_file.arff>: fichero de weka entrenado con acompanamiento
        <csv_file>: fichero con las etiquetas
...
for filename in sorted(csv.dic.keys(), key=str.lower):
    ...
    # prob accomp
    pa=lmidi.get_probabilites(train_a, filename_arff, debug)
    #---------------------------------------------------------------------------
    ...
    for t in m.tracks.keys():
        ...
        #if not oc and tag:
        print "   %2s   %2s   %3s   %3s   %3s  %4s %-9s  %s" % (t, selected, prob_track_m, prob_track_b, prob_track_a, oc, tag, m.tracks[t].name)
