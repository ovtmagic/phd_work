...
#
# ./multimoda_loo2.py <bass|melody|accomp> <bass|melody|accomp> <bass|melody|accomp> <threshold > <csv_file> <path_to_files>
#    <bass|melody|accomp>: main
#    <bass|melody|accomp>: other
#    <bass|melody|accomp>: other
#    <threshold>
...
def get_track_multimodal(prob_bass, prob_melody, prob_accomp, thres, m):
    # se pasan como argumentos las probabilidades p(B|i),  p(M|i) y p(A|i)
    # hay que calcular p(i|B), p(i|M) y p(i|A)
    prob_iB = lmidi.get_prob_iB(prob_bass, thres)
    prob_iM = lmidi.get_prob_iB(prob_melody, thres)
    prob_iA = lmidi.get_prob_iB(prob_accomp, thres)
    ...
    for i in prob_iB.keys():
        iB[i] = prob_iB[i]*(1.0-prob_iM[i])*(1.0-prob_iA[i])
    #x = 0.0
    #for i in prob_iB.keys():
    #    x = x + prob_iB[i]
...
def create_arff_file(one, all_files):
    # fichero arrf bass
    ...
    # fichero arrf accompaniment
    test_file = arff_header + arff_list_accomp[one]
    train_file = arff_header
    for i in all_files:
        train_file = train_file + arff_list_accomp[i] + "\n"
    write_file("tmp/a_test.arff",test_file)
    write_file("tmp/a_train.arff",train_file)
...
if (len(sys.argv) == 7):
    ...
    # teniendo en cuenta la informacion de estas pistas (melody y accomp)
    track_type_melody = sys.argv[2]
    track_type_accomp = sys.argv[3]
...
for filename in sorted(csv.dic.keys(), key=str.lower):
    ...
    class_ok_accomp = csv.get(filename, track_type_accomp)
    ...
    arff_list_accomp[filename] = midi_list[filename].get_arff(class_ok_accomp)
...
for i in loo_list:
    ...
    prob_accomp = lmidi.get_probabilites("tmp/a_train.arff", "tmp/a_test.arff", debug)
    bass_track = get_track_multimodal(prob_bass, prob_melody, prob_accomp, threshold, m)
