...
elif platform.system() in ('CYGWIN_NT-6.1-WOW64', 'CYGWIN_NT-6.1-WOW'):
...
class Midi:
    ...
    # pe: umbral
    # csv: fichro csv con las pistas de bajo etiquetadas (indica cual es la pista de bajo)
    def get_confusion_matrix(self, genre, arff_file, pe, csv, probabilities={}):
        # midi file name without path
        midi_file=self.filename.split("/")[-1]
        # the genre={bass|melody} track is selected
        track = self.get_selected_track(arff_file, midi_file, pe, probabilities)
        # error in selection is obtained
        return  csv.get_confusion_matrix(midi_file, track, genre)
