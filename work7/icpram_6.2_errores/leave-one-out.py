...
accuracy = 100*total_errors[0]/(total_errors[0]+total_errors[1]+total_errors[2]+total_errors[3])

print "%s;%s;%.2f%%,%s;%s;%s;%s;%s;%s;%s;%s;%.2f;%.2f;%.2f" % (csv_filename.split("/")[-1], track_type, accuracy, total_errors[0], total_errors[1], total_errors[2], total_errors[3], confusion_matrix['TP'], confusion_matrix['FP'], confusion_matrix['FN'],  confusion_matrix['TN'], precision, recall, f_m)
